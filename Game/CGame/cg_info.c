...
===========================================================================
*/
// cg_info.c -- display information while data is being loaded
#include "cg_local.h"
...
static void CG_DrawLoadingIcons(void){
	...
	for(n=0;n<loadingPlayerIconCount;n++){
		...
		CG_DrawSmallStringCustom(42, y+10, 11, 11, loadingPlayerNames[n], 1.f, 6);
...
void CG_LoadingClient(int clientNum){
	...
	if(loadingPlayerIconCount < MAX_LOADING_PLAYER_ICONS){
		...
		Com_sprintf(iconName, MAX_QPATH, "players//%s/icon_%s", model, skin);
		strcpy(loadingPlayerNames[loadingPlayerIconCount],name);
		loadingPlayerIcons[loadingPlayerIconCount] = trap_R_RegisterShaderNoMip(iconName);
		// FIXME: we might want to at least pop up a warning message for invalid icons.
		if(!loadingPlayerIcons[loadingPlayerIconCount]){
			Com_sprintf(iconName, MAX_QPATH, "players//%s/icon_%s", DEFAULT_MODEL, "default");
			loadingPlayerIcons[loadingPlayerIconCount] = trap_R_RegisterShaderNoMip(iconName);
		}
		if(loadingPlayerIcons[loadingPlayerIconCount]) loadingPlayerIconCount++;
	}
...
void CG_DrawInformation( void ) {
	...
	// FIXME: we might want to at least pop up a warning message for invalid levelshots.
