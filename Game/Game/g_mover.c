...
gentity_t* G_TestEntityPosition(gentity_t *ent){
	...
	vec3_t traceStart;
	VectorCopy(ent->client ? ent->client->ps.origin : ent->s.pos.trBase,traceStart);
	trap_Trace(&tr,traceStart,ent->r.mins,ent->r.maxs,traceStart,ent->s.number,mask);
...
qboolean G_TryPushingEntity(gentity_t* check,gentity_t* pusher,vec3_t move,vec3_t amove){
	...
	gentity_t*	block;
	vec3_t		origin;
	VectorCopy(check->client ? check->client->ps.origin : check->s.pos.trBase,origin);
	...
	VectorSubtract(origin,pusher->r.currentOrigin,org);
	VectorCopy(org,org2);
	...
	if(!block){
		VectorCopy(origin,check->r.currentOrigin);
...
qboolean G_CheckProxMinePosition(gentity_t *check){
	...
	return tr.startsolid || tr.fraction < 1 ? qfalse : qtrue;
}
...
qboolean G_MoverPush(gentity_t* pusher,vec3_t move,vec3_t amove,gentity_t** obstacle){
	...
	*obstacle = NULL;
	...
	if(pusher->r.currentAngles[0] || pusher->r.currentAngles[1] || pusher->r.currentAngles[2] || amove[0] || amove[1] || amove[2]){
		...
		for(i=0;i<3;i++){
			if(move[i] > 0){totalMaxs[i] += move[i];}
			else{totalMins[i] += move[i];}
		}
	}
	...
	for(e=0;e<listedEntities;e++){
		...
		if(check->s.groundEntityNum != pusher->s.number){
			...
			if(!G_TestEntityPosition(check)){continue;}
		}
		// the entity needs to be pushed
		if(G_TryPushingEntity(check,pusher,move,amove)){continue;}
		// the move was blocked an entity
		// bobbing entities are instant-kill and never get blocked
		if(pusher->s.pos.trType == TR_SINE || pusher->s.apos.trType == TR_SINE){continue;}
...
void InitMover(gentity_t* ent){
	...
	if(lightSet || colorSet){
		int	r = Com_Clamp(0,255,color[0] * 255);
		int g = Com_Clamp(0,255,color[1] * 255);
		int b = Com_Clamp(0,255,color[2] * 255);
		int i = Com_Clamp(0,255,light / 4);
	...
	if(ent->s.pos.trDuration <= 0){ent->s.pos.trDuration = 1;}
}
/*
===============================================================================
DOOR
...
static void Touch_DoorTriggerSpectator(gentity_t* ent,gentity_t* other){
	...
	origin[axis] = fabs(origin[axis] - doorMax) < fabs(origin[axis] - doorMin) ? doorMin - 10 : doorMax + 10;
...
void Touch_Plat(gentity_t* ent,gentity_t* other){
	if(!other->client || other->client->ps.powerLevel[plCurrent] <= 0){return;}
	// delay return-to-pos1 by one second
	if(ent->moverState == MOVER_POS2){ent->nextthink = level.time + 1000;}
}
/*
==============
...
void SpawnPlatTrigger(gentity_t* ent){
	...
	for(int i=0;i<2;i++){
		if(tmax[i] <= tmin[i]){
			tmin[i] = ent->pos1[i] + (ent->r.mins[i] + ent->r.maxs[i]) * 0.5;
			tmax[i] = tmin[i] + 1;
		}
	}
...
void SP_func_plat(gentity_t* ent){
	...
	if(!ent->targetname){SpawnPlatTrigger(ent);}
}
/*
===============================================================================
BUTTON
...
void SP_func_button(gentity_t* ent){
	...
	VectorMA(ent->pos1,distance,ent->movedir,ent->pos2);
	// shootable button
	if(ent->powerLevelTotal){ent->takedamage = qtrue;}
	// touchable button
	else{ent->touch = Touch_Button;}
	InitMover(ent);
}
/*
===============================================================================
...
void Reached_Train(gentity_t* ent){
	...
	speed = next->speed ? next->speed : ent->speed;
	if(speed<1){speed = 1;}
	// calculate duration
	VectorSubtract(ent->pos2,ent->pos1,move);
...
void SP_func_rotating(gentity_t* ent){
	...
	if(ent->spawnflags & 4){ent->s.apos.trDelta[2] = ent->speed;}
	else if(ent->spawnflags & 8){ent->s.apos.trDelta[0] = ent->speed;}
	else{ent->s.apos.trDelta[1] = ent->speed;}
	if(!ent->damage){ent->damage = 2;}
	trap_SetBrushModel(ent,ent->model);
	InitMover(ent);
	VectorCopy(ent->s.origin,ent->s.pos.trBase);
...
void SP_func_bobbing(gentity_t* ent){
	...
	if(ent->spawnflags & 1){ent->s.pos.trDelta[0] = height;}
	else if(ent->spawnflags & 2){ent->s.pos.trDelta[1] = height;}
	else{ent->s.pos.trDelta[2] = height;}
}
/*
===============================================================================
PENDULUM
...
void SP_func_pendulum(gentity_t* ent){
	...
	freq = 1/(M_PI*2) * sqrt(1000/(3*length));
