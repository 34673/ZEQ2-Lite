...
typedef struct beamTableElem_s{
	struct beamTableElem_s *prev;
	struct beamTableElem_s *next;
	vec3_t pos;
	vec3_t tangent;
}beamTableElem_t;
typedef struct{
...
static void CG_WipeBeamTable(beamTable_t *beamTable){
	if(!beamTable->alreadyWiped){
		...
		for(int i=0;i<MAX_BEAMTABLE_SIZE-1;i++){
...
static beamTableElem_t *CG_AllocBeamTableElem(beamTable_t *beamTable){
	...
	tableElem = beamTable->table_freeList;
	if(!tableElem){
...
void CG_BeamTableUpdate(centity_t *cent,float width,qhandle_t shader,char *tagName){
	...
	int clientNumber = cent->currentState.clientNum;
	...
	currentTable = cent->currentState.weapon > ALTWEAPON_OFFSET ? &beamTableAlternate[clientNumber] : &beamTablePrimary[clientNumber];
...
*/
static void CG_BezierMidPoints(const vec3_t startPos,const vec3_t endPos,const vec3_t startTan,const vec3_t endTan,vec3_t midPos1,vec3_t midPos2){
	...
	float tangentScale = Distance(startPos,endPos) / 2.0f;
...
*/
static void CG_BezierPoint(const vec3_t point0,const vec3_t point1,const vec3_t point2,const vec3_t point3,float t,vec3_t outPoint,vec3_t outTangent){
	float T[4];
	float H[4];
	vec3_t tmp[4];
...
static int CG_TessCount(const beamTableElem_t *startElem,const beamTableElem_t *endElem){
	...
	r_beamDetail.value = Com_Clamp(0.0f,4.0f,r_beamDetail.value);
	detail = r_beamDetail.value * 10;
	if(r_beamDetail.value >= 4 && tangentRatio < 1){tangentRatio = 1;}
	else if(r_beamDetail.value == 3 && tangentRatio < 0.5f){tangentRatio = 0.5f;}
	else if(r_beamDetail.value == 2 && tangentRatio < 0.25f){tangentRatio = 0.25f;}
	else if(r_beamDetail.value == 1 && tangentRatio < 0.125f){tangentRatio = 0.125f;}
	// On small distances always use the maximum tangent ratio
	if(dist < 100){tangentRatio = 2;}
...
static void CG_DrawBeamTable(int clientNum,qboolean alternate){
	...
	// Do we work on the alternate fire tables, or the primary fire ones?
	currentTable = alternate ? &beamTableAlternate[clientNum] : &beamTablePrimary[clientNum];
	// Retrieve the correct starter point from all model handles
	// If the tag can not be found, wipe the table and don't display the beam
	if(!CG_TryLerpPlayerTag(&cg_entities[clientNum],currentTable->tagName,&orient)){
	...
	while(1){
		...
		int tessSize = CG_TessCount(prevElem,currentElem);
		// get the midpoints for this segment
		CG_BezierMidPoints(prevElem->pos,currentElem->pos,prevElem->tangent,currentElem->tangent,midPos1,midPos2);
		// generate the tesselations
		for(int i=1;i<=tessSize;i++){
			float t = (float)i / (float)tessSize;
		...
		if(currentElem == &currentTable->table_activeList){break;}
