...
OPUSDIR=Engine/opus
OPUSFILEDIR=Engine/opusfile
ZDIR=Engine/zlib
TOOLSDIR=Engine/tools
...
AUTOUPDATERSRCDIR=Engine/autoupdater
...
ifneq (,$(findstring "$(PLATFORM)", "linux" "gnu_kfreebsd" "kfreebsd-gnu" "gnu"))
  BASE_CFLAGS = $(LANGUAGE_STANDARD) -Wall -fno-strict-aliasing -Wimplicit -Wstrict-prototypes \
...
ifdef MINGW
  ...
  ifeq ($(ARCH),x86)
    BASE_CFLAGS += -Wl,--large-address-aware
  endif
...
ifeq ($(PLATFORM),freebsd)
  ...
  # flags
  BASE_CFLAGS = \
    $(LANGUAGE_STANDARD) -Wall -fno-strict-aliasing -Wimplicit -Wstrict-prototypes \
...
CGOBJ_ = \
  ...
  $(B)/$(BASEGAME)/CGame/cg_animations.o \
  $(B)/$(BASEGAME)/CGame/cg_animationConfig.o \
  ...
  $(B)/$(BASEGAME)/CGame/cg_motionblur.o \
  \
...
$(B)/$(BASEGAME)/vm/cgame.qvm: $(CGVMOBJ) $(CGDIR)/cg_syscalls.asm $(Q3ASM)
	$(echo_cmd) "Q3ASM $@"
	$(Q)$(Q3ASM) -m -o $@ $(CGVMOBJ) $(CGDIR)/cg_syscalls.asm
...
$(B)/$(BASEGAME)/vm/game.qvm: $(GVMOBJ) $(GDIR)/g_syscalls.asm $(Q3ASM)
	$(echo_cmd) "Q3ASM $@"
	$(Q)$(Q3ASM) -m -o $@ $(GVMOBJ) $(GDIR)/g_syscalls.asm
...
$(B)/$(BASEGAME)/vm/ui.qvm: $(UIVMOBJ) $(UIDIR)/ui_syscalls.asm $(Q3ASM)
	$(echo_cmd) "Q3ASM $@"
	$(Q)$(Q3ASM) -m -o $@ $(UIVMOBJ) $(UIDIR)/ui_syscalls.asm
...
OBJ = $(Q3OBJ) $(Q3ROBJ) $(Q3DOBJ) $(JPGOBJ) \
  $(GOBJ) $(CGOBJ) $(UIOBJ) \
...
ifneq ($(BUILD_SERVER),0)
	@if [ -f $(BR)/$(SERVERBIN)$(FULLBINEXT) ]; then \
		$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/$(SERVERBIN)$(FULLBINEXT) $(COPYBINDIR)/$(SERVERBIN)$(BINEXT); \
	fi
endif
