...
typedef struct PSys_Emitter_s{
	...
	float					radius,		// Spawn radius
							offset,		// Spawn offset ( radius types only )
							posJit;		// Amount of jitter to put into spawn position

	int						lifeTime,	// Lifetime for the emitter
							startTime,	// Time the emitter was spawned
							waitTime,	// Time to wait between spawn sessions, 0 means only spawn once
							lastTime,	// Last time a spawn session took place
							initTime;	// Time to wait before the first spawn session
...
typedef struct PSys_Force_s{
	...
	struct PSys_Force_s		*prev, *next,
							*prev_local, *next_local; // Same, but local within the system
	// IMPORTANT: type -MUST- be the next field, or a memory copy error
	//            will occur within the loading from cache code
	...
	float					AOIrange[3],// Represents the x,y,z ratios of the force's area of influence.
							...
							falloff,	// Amount of area over which the force declines from full to no effect.
							value,		// Amount of force applied
							pullIn;		// Only used for swirl type. Determines inward pull to center of swirl
}PSys_Force_t;
...
typedef struct PSys_Particle_s{
	...
	*/
	...
	vec3_t					rayOffset,	// original offset from emitter
							rayOrigin;	// last updated starting point of ray
}PSys_Particle_t;
