#include "g_local.h"
// update the radar once every X milliseconds
#define RADAR_UPDATE_TIME 5000
//global storage for player positions
radar_t g_playerOrigins[MAX_CLIENTS];
void G_RadarUpdateCS(void){
	int i;
	int valid_count;
	gentity_t* ent;
	playerState_t* ps;
	// make sure our command string is large enough for all the data
	...
	if(level.time - level.lastRadarUpdateTime > RADAR_UPDATE_TIME){
		...
		for(i=0;i<g_maxclients.integer;i++){
			...
			if(ent->client->pers.connected != CON_CONNECTED){g_playerOrigins[i].valid = qfalse;}
			else if(!ent->inuse){g_playerOrigins[i].valid = qfalse;}
			else if(ent->client->ps.powerLevel[plCurrent] <= 0){g_playerOrigins[i].valid = qfalse;}
			else{
		...
		for(i=0;i<g_maxclients.integer;i++){
			...
			if(ent->inuse){trap_SendServerCommand(ent-g_entities,cmd);}
		}
	}
}
