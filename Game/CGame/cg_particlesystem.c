...
static qboolean PSys_UpdateOrientation(PSys_System_t *system, PSys_Orientation_t *orient){
	if(orient->entity){
		...
		if(orient->linkAuraState && !CG_FrameHist_HasAura(orient->entity->currentState.number)) return qfalse;
		// Check if we should link against the weapon
		if(orient->linkWeapon){
			// If we switched weapons
			if(CG_FrameHist_IsWeaponNr(orient->entity->currentState.number) != orient->weaponNr) return qfalse;
	...
	if(VectorLength(orient->dir) > 0.f){
...
static void PSys_UpdateForces(PSys_System_t *system){
	...
	for(;force != &(system->forces); force = next){
		...
		if(!PSys_UpdateOrientation(system, &(force->orientation))) PSys_FreeForce(force);
	}
}
...
static void PSys_UpdateEmitters(PSys_System_t *system){
	...
	for(;emitter != &(system->emitters); emitter = next){
		...
		while(((emitter->lastTime + emitter->waitTime) <= cg.time) && (trace.fraction < 1.f)){
			PSys_Particle_t *particle;
			...
			for(i=0; i<emitter->amount; i++){
				...
				if(particle->mass)	particle->invMass = 1 / particle->mass;
				else				particle->invMass = 0;
	...
	switch(force->AOItype){
		...
		case AOI_SPHERE:
			...
			if(force->falloff){
				...
				if(distance[0] > force->AOIrange[0]) lerp = 0;
				else lerp = 1;
			}
		...
		case AOI_CYLINDER:
			...
			if(force->falloff){
				...
				if(distance[0] > force->AOIrange[0] || distance[1] > force->AOIrange[1]) lerp = 0;
				else lerp = 1;
			}
			return(lerp * sourceVal);
			break;
	...
	for(;force != &(system->forces); force = next){
		...
		switch(force->type){
			...
			case FTYPE_SPHERICAL:
				...
				if(VectorNormalize(sphereForce) == 0.f)
			...
			case FTYPE_SWIRL:
				...
				if(VectorNormalize(swirlOut) == 0.f) break;
...
static void PSys_IntegrateParticle(PSys_Particle_t *particle, float timeStepSquare, float timeStepCorrected){
	...
	if(particle->mass) VectorAdd(particle->forceAccum, particle->accelAccum, particle->forceAccum);
...
static qboolean PSys_ApplyPlaneConstraint(PSys_System_t *system, float value){
	...
	for(;particle != &(system->particles); particle = next){
		...
		if(trace.fraction < 1.f){
			...
			if(value == 0.f && cg_particlesStop.value){
				PSys_FreeParticle(particle);
				continue;
			}
			...
			// check for stop
			if(trace.plane.normal[2] > .2f && VectorLength(v) < MIN_BOUNCE_DELTA){
				if(VectorLength(v) > 0.f && VectorLength(v) < .6f){
					VectorSet(v, 0, 0, 0);
					particle->mass = 0.f;
				}
...
static void PSys_UpdateSystems(void){
	...
	timeStep = cg.frametime * .01f;
	timeStepSquare = timeStep * timeStep;
	if(!PSys_LastTimeStep) PSys_LastTimeStep = .01f; // A minimal is needed to avoid division by zero!
	...
	for(;system != &(PSys_Systems_inuse); system = next){
		...
		if((system->particles.prev_local == &(system->particles)) && (system->emitters.prev_local  == &(system->emitters))){
	...
	for(;system != &(PSys_Systems_inuse); system = next_s){
		...
		for(;particle != &(system->particles); particle = next_p){
			...
			if(lifetime_cur < particle->scale.startTime){
				if(particle->scale.startTime)
					lerp = (float)lifetime_cur / (float)particle->scale.startTime;
				else lerp = 1;
				if(lerp < 0) lerp = 0;
				if(lerp > 1) lerp = 1;
				lerpedScale = lerp * lerpedScale + particle->scale.startVal * (1.f - lerp);
			}
			else if(lifetime_cur > (lifetime_end - particle->scale.endTime)){
				if(particle->scale.endTime)
					lerp = (float)(lifetime_end - lifetime_cur) / (float)particle->scale.endTime;
				else lerp = 1;
				if(lerp < 0) lerp = 0;
				if(lerp > 1) lerp = 1;
				lerpedScale = lerp * lerpedScale + particle->scale.endVal * (1.f - lerp );
			}
			// Lerp rgba
			if(lifetime_cur < particle->rgba.startTime){
				if(particle->rgba.startTime)
					lerp = (float)lifetime_cur / (float)particle->rgba.startTime;
				else lerp = 1;
				if(lerp < 0) lerp = 0;
				if(lerp > 1) lerp = 1;
				lerpedRGBA[0] = lerp * lerpedRGBA[0] + particle->rgba.startVal[0] * (1.f - lerp);
				lerpedRGBA[1] = lerp * lerpedRGBA[1] + particle->rgba.startVal[1] * (1.f - lerp);
				lerpedRGBA[2] = lerp * lerpedRGBA[2] + particle->rgba.startVal[2] * (1.f - lerp);
				lerpedRGBA[3] = lerp * lerpedRGBA[3] + particle->rgba.startVal[3] * (1.f - lerp);
			}
			else if(lifetime_cur > (lifetime_end - particle->rgba.endTime)){
				if(particle->rgba.endTime)
					lerp = (float)(lifetime_end - lifetime_cur) / (float)particle->rgba.endTime;
				else lerp = 1;
				if(lerp < 0) lerp = 0;
				if(lerp > 1) lerp = 1;
				lerpedRGBA[0] = lerp * lerpedRGBA[0] + particle->rgba.endVal[0] * (1.f - lerp);
				lerpedRGBA[1] = lerp * lerpedRGBA[1] + particle->rgba.endVal[1] * (1.f - lerp);
				lerpedRGBA[2] = lerp * lerpedRGBA[2] + particle->rgba.endVal[2] * (1.f - lerp);
				lerpedRGBA[3] = lerp * lerpedRGBA[3] + particle->rgba.endVal[3] * (1.f - lerp);
			}
			// Lerp rotation
			if(lifetime_cur < particle->rotate.startTime){
				if(particle->rotate.startTime)
					lerp = (float)lifetime_cur / (float)particle->rotate.startTime;
				else lerp = 1;
				if(lerp < 0) lerp = 0;
				if(lerp > 1) lerp = 1;
				lerpedRotation[0] = lerp * lerpedRotation[0] + particle->rotate.startVal[0] * (1.f - lerp);
				lerpedRotation[1] = lerp * lerpedRotation[1] + particle->rotate.startVal[1] * (1.f - lerp);
				lerpedRotation[2] = lerp * lerpedRotation[2] + particle->rotate.startVal[2] * (1.f - lerp);
				lerpedRotation[3] = lerp * lerpedRotation[3] + particle->rotate.startVal[3] * (1.f - lerp);
			}
			else if(lifetime_cur > (lifetime_end - particle->rotate.endTime)){
				if(particle->rotate.endTime)
					lerp = (float)(lifetime_end - lifetime_cur) / (float)particle->rotate.endTime;
				else lerp = 1;
				if(lerp < 0) lerp = 0;
				if(lerp > 1) lerp = 1;
				lerpedRotation[0] = lerp * lerpedRotation[0] + particle->rotate.endVal[0] * (1.f - lerp);
				lerpedRotation[1] = lerp * lerpedRotation[1] + particle->rotate.endVal[1] * (1.f - lerp);
				lerpedRotation[2] = lerp * lerpedRotation[2] + particle->rotate.endVal[2] * (1.f - lerp);
				lerpedRotation[3] = lerp * lerpedRotation[3] + particle->rotate.endVal[3] * (1.f - lerp);
			}
			switch(particle->rType){
				case RTYPE_DEFAULT:
					...
					if(!particle->model){
						...
						if((particle->oldPosition != particle->position) && ((lerpedRotation[0] || lerpedRotation[1] || lerpedRotation[2]) > 0))
						...
						if(particle->oldPosition != particle->position){
							if(lerpedRotation[0]) lerpedRotation[0] = particle->position[0]/* * lerpedRotation[0] / 100*/;
							if(lerpedRotation[1]) lerpedRotation[1] = particle->position[1]/* * lerpedRotation[1] / 100*/;
							if(lerpedRotation[2]) lerpedRotation[2] = particle->position[2]/* * lerpedRotation[2] / 100*/;
						}
...
*/
void PSys_SpawnCachedSystem(char* systemName, vec3_t origin, vec3_t *axis, centity_t *cent, char* tagName, qboolean auraLink, qboolean weaponLink){
	...
	if(axis)	AxisCopy(axis, system->rootAxis);
		...
		switch(cache->members[i].type){
			case MEM_EMITTER:
				...
				if(emitter->orientation.autoLink){
					if(cent){
						if(tagName) Q_strncpyz(emitter->orientation.tagName, tagName, sizeof(emitter->orientation.tagName));
			...
			case MEM_FORCE:
				...
				if(force->orientation.autoLink){
					if(cent){
						if(tagName) Q_strncpyz(force->orientation.tagName, tagName, sizeof(force->orientation.tagName));
