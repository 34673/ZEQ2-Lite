...
// cg_local.h is included in the parser header
...
//Scans the next symbol in the scanner's loaded scriptfile.
qboolean CG_weapGfx_NextSym(cg_weapGfxScanner_t *scanner, cg_weapGfxToken_t *token){
	...
	while(1){
		...
		// Skip /* */ comments
			...
			scanner->pos += 2;
			while(*scanner->pos && (scanner->pos[0] != '*' || scanner->pos[1] != '/')){scanner->pos++;}
	...
	if((*scanner->pos >= '0') && (*scanner->pos <= '9')){
		...
		if(dot){token->tokenSym = TOKEN_FLOAT;}
		else{token->tokenSym = TOKEN_INTEGER;}
		token->stringval[len] = '\0';
	...
	// Handle keywords
	{
		...
		for(;strcmp(cg_weapGfxCategories[i], "");i++){
		...
		for(i=0;strcmp(cg_weapGfxFields[i].fieldname, ""); i++){
			// NOTE: Need to do this comparison case independantly. Q_stricmp is case
...
qboolean CG_weapGfx_LoadFile(cg_weapGfxScanner_t *scanner, char *filename){
	...
	if(cg_verboseParse.integer){CG_Printf("Scriptfile '%s' has been read into memory.\n", filename);}
	return qtrue;
}
