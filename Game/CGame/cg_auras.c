...
static qboolean CG_Aura_ArrangeConvexHull(auraTag_t *points, int *nr_points){
	...
	auraTag_t	pivot, behind, infront, vecPoint, buffer[MAX_AURATAGS+1];
	qboolean	rightTurn;
	
	amount = *nr_points;	
	if(amount == 3) return qtrue;	// Already a convex hull
	else if(amount < 3) return qfalse;
	pivotIndex = 0;
	...
	do{
		...
		if(!index) rightTurn = qtrue;
		else{
			behind = points[index-1];
			if(index == amount-1) infront = pivot;
			else infront = points[index+1];
			...
			if(((behind.pos_screen[0] - points[index].pos_screen[0]) * (infront.pos_screen[1] - points[index].pos_screen[1]) -
				...
				rightTurn = qtrue;
			else rightTurn = qfalse;
		}
		
		if(rightTurn) index++;	// point is currently considered part of the hull
		else{
...
static void CG_Aura_SetHullAttributes( auraState_t *state){
	int			index, behind, infront, nr_points;
	vec3_t		line_behind, line_infront, viewLine,
				temp_behind, temp_infront;
	float		circumference;
	auraTag_t	*points;
...
static void CG_LerpSpike(auraState_t *state, auraConfig_t *config, int spikeNr, float alphaModulate){
	int		start, end;
	float	progress_pct, lerpTime, lerpModulate, lerpBorder, baseBorder, lerpSize;
	vec3_t	viewLine, lerpPos, lerpNormal, lerpDir, endPos, basePos;
	vec4_t	lerpColor;
	...
	VectorSet(lerpNormal,0,0,0);
	VectorMA(lerpNormal, 1-progress_pct, state->convexHull[start].normal, lerpNormal);
...
static void CG_Aura_ConvexHullRender(centity_t *player, auraState_t *state, auraConfig_t *config){
	...
	if(!(state->isActive || (state->modulate > 0))) return;
...
static void CG_Aura_DimLight(centity_t *player, auraState_t *state, auraConfig_t *config){
	if(state->isActive){
		if(state->lightAmt < config->lightMax){
			state->lightAmt += config->lightGrowthRate *(cg.frametime / 25.f);
			if(state->lightAmt > config->lightMax)
		...
		if(state->lightAmt > config->lightMin){
			state->lightAmt -= config->lightGrowthRate *(cg.frametime / 50.f);
			if(state->lightAmt < config->lightMin)
	...
	if(state->modulate > 1.f) state->modulate = 1.f;
}
...
void parseAura(char *path,auraConfig_t *aura){
	...
	int 			i, fileLength;
	char 			*token, *parse, fileContents[32000];
...
localEntity_t *CG_AuraSpike(const vec3_t p, const vec3_t vel, float radius, float duration, int startTime, int fadeInTime, int leFlags, centity_t *player){
	...
	if(fadeInTime > startTime)
		le->lifeRate = 1.f / (le->endTime - le->fadeInTime);
	else 
