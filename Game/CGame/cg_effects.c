...
void CG_BubbleTrail(vec3_t start, vec3_t end, float spacing){
	...
	for(;i<len;i+=spacing){
		...
		le->lifeRate = 1.f / (le->endTime -le->startTime);
		re = &le->refEntity;
		re->shaderTime = cg.time / 1000.f;
		re->reType = RT_SPRITE;
		re->rotation = 0;
		re->radius = 3;
...
localEntity_t *CG_SmokePuff(const vec3_t p, const vec3_t vel, 
	...
	re->radius = radius;
	re->shaderTime = startTime / 1000.f;
	le->leType = LE_MOVE_SCALE_FADE;
	le->startTime = startTime;
	le->fadeInTime = fadeInTime;
	le->endTime = startTime +duration;
	if(fadeInTime > startTime)	le->lifeRate = 1.f / (le->endTime -le->fadeInTime);
	else						le->lifeRate = 1.f / (le->endTime -le->startTime);
	le->color[0] = r;
	le->color[1] = g; 
	le->color[2] = b;
	le->color[3] = a;
...
void CG_DirtPush(vec3_t org, vec3_t dir, int size){
	...
	re->shaderTime = le->startTime / 1000.f;
	AxisClear(re->axis);
...
void CG_WaterRipple(vec3_t org, int size, qboolean single){
	...
	if(single){
		...
		re->hModel = cgs.media.waterRippleSingleModel;
			 if(random()*3 < 1) trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashSmall1);
		else if(random()*3 < 2) trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashSmall2);
		else					trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashSmall3);
	}
	else{
		...
		if(size > 0 && size < 10){
				 ...
				 if(random()*4 < 1) trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashMedium1);
			else if(random()*4 < 2) trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashMedium2);
			else if(random()*4 < 3) trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashMedium3);
			else					trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashMedium4);
		}
		else if(size >= 25) trap_S_StartSound(org,ENTITYNUM_NONE,CHAN_AUTO,cgs.media.waterSplashLarge1);
		else if(size >= 50){
			if(random()*2 < 1)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashExtraLarge1);
			else				trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.waterSplashExtraLarge2);
		}
	}
	// bias the time so all shader effects start correctly
	re->shaderTime = le->startTime / 1000.f;
...
void CG_WaterSplash(vec3_t org, int size){
	...
	if((ang >= 70.f && ang <= 110.f) || (ang >= 250.f && ang <= 290.f)) 
		ang = rand() % 69;
...
void CG_LightningEffect(vec3_t org, clientInfo_t *ci, int tier){
	...
	if(r[0] > 58){
		...
		re->reType = RT_SPRITE;
		re->radius = le->radius;
		re->shaderRGBA[0] = ci->auraConfig[tier]->lightningColor[0] *0xff;
		re->shaderRGBA[1] = ci->auraConfig[tier]->lightningColor[1] *0xff;
		re->shaderRGBA[2] = ci->auraConfig[tier]->lightningColor[2] *0xff;
		re->shaderRGBA[3] = 0xff;
		...
		re->origin[1] -= r[5];
		re->origin[2] -= r[6];
			 if(random()*7 < 1)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound1 );
		else if(random()*7 < 2)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound2 );
		else if(random()*7 < 3)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound3 );
		else if(random()*7 < 4)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound4 );
		else if(random()*7 < 5)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound5 );
		else if(random()*7 < 6)	trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound6 );
		else					trap_S_StartSound( org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound7 );
	}
}
...
void CG_BigLightningEffect(vec3_t org){
	...
	VectorCopy(org, re->origin);
	re->origin[2] += 16;
	if(r < 2)	re->customShader = cgs.media.auraLightningSparks1;
	if(r > 198) re->customShader = cgs.media.auraLightningSparks2;
	if(random() > 0 && random() < .1f)	 trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound1);
	if(random() > .1f && random() < .2f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound2);
	if(random() > .2f && random() < .3f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound3);
	if(random() > .3f && random() < .4f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound4);
	if(random() > .4f && random() < .5f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound5);
	if(random() > .5f && random() < .6f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound6);
	if(random() > .6f && random() < .7f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound7);
	if(random() > .7f && random() < .8f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound1);
	if(random() > .8f && random() < .9f) trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound2);
	if(random() > .9f && random() < 1)	 trap_S_StartSound(org, ENTITYNUM_NONE, CHAN_AUTO, cgs.media.bigLightningSound3);
}
...
void CG_SpeedMeleeEffect(vec3_t org, int tier){
	...
	if(r[0] > 50){
		...
		le->endTime = cg.time +100;
			 if(tier == 7)	le->radius = 2048;
		else if(tier == 6)	le->radius = 1024;
		else if(tier == 5)	le->radius = 512;
		else if(tier == 4)	le->radius = 256;
		else if(tier == 3)	le->radius = 128;
		else if(tier == 2)	le->radius = 64;
		else				le->radius = 32;
		le->lifeRate = 1.f / (le->endTime -le->startTime);
		le->color[0] = le->color[1] = le->color[2] = le->color[3] = 1.f;
		re = &le->refEntity;
...
void CG_PowerStruggleEffect( vec3_t org, int size ) {
	...
	le2->lifeRate = 1.f / (le2->endTime -le2->startTime);
	le2->color[0] = le2->color[1] = le2->color[2] = le2->color[3] = 1.f;
	re2 = &le2->refEntity;
...
localEntity_t *CG_MakeExplosion(vec3_t origin, vec3_t dir, 
	...
	// bias the time so all shader effects start correctly
	ex->refEntity.shaderTime = ex->startTime / 1000.f;
	ex->refEntity.hModel = hModel;
	ex->refEntity.customShader = shader;
	// set origin
	VectorCopy(newOrigin, ex->refEntity.origin);
	VectorCopy(newOrigin, ex->refEntity.oldorigin);
	ex->color[0] = ex->color[1] = ex->color[2] = 1.f;
	return ex;
}
...
void CG_MakeUserExplosion(vec3_t origin, vec3_t dir, cg_userWeapon_t *weaponGraphics, int powerups, int number) {
	...
	if(!(weaponGraphics->explosionModel && weaponGraphics->explosionSkin)){
		if(weaponGraphics->explosionShader){
			...
			expShell->refEntity.radius = explosionScale *4;	//weaponGraphics->explosionSize;
			// randomly rotate sprite orientation
			...
			expShell->lifeRate = 1.f / (expShell->endTime -expShell->startTime);
			// screen fade
			...
			if(explosionScale > 0){
				...
				cg.screenFlashFadeAmount = 1.f -(distance / (explosionScale*100));
				if(cg.screenFlashFadeAmount < 0)	cg.screenFlashFadeAmount = 0;
				if(cg.screenFlashFadeAmount > .5f)	cg.screenFlashFadeAmount = .5f;
			}
			...
			CG_AddEarthquake(origin, 
				...
				, explosionScale/*weaponGraphics->explosionSize*/ / 2);
			// bias the time so all shader effects start correctly
			expShell->refEntity.shaderTime = expShell->startTime / 1000.f;
		...
		if(explosionScale > 0){
			...
			cg.screenFlashFadeAmount = 1.f -(distance / (explosionScale *100));
			if(cg.screenFlashFadeAmount < 0)	cg.screenFlashFadeAmount = 0;
			if(cg.screenFlashFadeAmount > 1.f)	cg.screenFlashFadeAmount = 1.f;
		}
		// create a camera shake
		CG_AddEarthquake(origin, 
	...
	if(weaponGraphics->shockwaveModel && weaponGraphics->shockwaveSkin){
		...
		expShock->endTime = expShock->startTime +(weaponGraphics->explosionTime / 1.5f);
		expShock->lifeRate = 1.f / (expShock->endTime -expShock->startTime);
/*
...
*/
		// bias the time so all shader effects start correctly
		expShock->refEntity.shaderTime = expShock->startTime / 1000.f;
		// disable this Dlight
		expShock->light = 0;
	}
...
void CG_CreateStraightBeamFade(vec3_t start, vec3_t end, cg_userWeapon_t *weaponGraphics){
	...
	beam->lifeRate = 1.f / (beam->endTime -beam->startTime);
	beam->refEntity.shaderTime = beam->startTime / 1000.f;
}
