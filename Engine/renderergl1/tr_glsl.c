		...
		if (!Q_stricmp(token, "uniform")) {
			...
			if (!Q_stricmp(token, "int")) {
				...
				if (!Q_stricmp(token, "u_AlphaGen;")) {
					program->u_AlphaGen = qglGetUniformLocation(program->program, "u_AlphaGen");
				} else if (!Q_stricmp(token, "u_ColorGen;")) {
					program->u_ColorGen = qglGetUniformLocation(program->program, "u_ColorGen");
				} else if (!Q_stricmp(token, "u_Greyscale;")) {
					program->u_Greyscale = qglGetUniformLocation(program->program, "u_Greyscale");
				} else if (!Q_stricmp(token, "u_TCGen0;")) {
					program->u_TCGen0 = qglGetUniformLocation(program->program, "u_TCGen0");
				} else if (!Q_stricmp(token, "u_TCGen1;")) {
					program->u_TCGen1 = qglGetUniformLocation(program->program, "u_TCGen1");
				} else if (!Q_stricmp(token, "u_TexEnv;")) {
					program->u_TexEnv = qglGetUniformLocation(program->program, "u_TexEnv");
				} else {
				...
				if (!Q_stricmp(token, "u_IdentityLight;")) {
					program->u_IdentityLight = qglGetUniformLocation(program->program, "u_IdentityLight");
				} else if (!Q_stricmp(token, "u_LightDistance;")) {
					program->u_LightDistance = qglGetUniformLocation(program->program, "u_LightDistance");
				...
				if (!Q_stricmp(token, "u_Texture0;")) {
					program->u_Texture0 = qglGetUniformLocation(program->program, "u_Texture0");
					R_GLSL_SetUniform_Texture0(program, 0);
				} else if (!Q_stricmp(token, "u_Texture1;")) {
					program->u_Texture1 = qglGetUniformLocation(program->program, "u_Texture1");
					R_GLSL_SetUniform_Texture1(program, 1);
				} else if (!Q_stricmp(token, "u_Texture2;")) {
					program->u_Texture2 = qglGetUniformLocation(program->program, "u_Texture2");
					R_GLSL_SetUniform_Texture2(program, 2);
				} else if (!Q_stricmp(token, "u_Texture3;")) {
					program->u_Texture3 = qglGetUniformLocation(program->program, "u_Texture3");
					R_GLSL_SetUniform_Texture3(program, 3);
				} else if (!Q_stricmp(token, "u_Texture4;")) {
					program->u_Texture4 = qglGetUniformLocation(program->program, "u_Texture4");
					R_GLSL_SetUniform_Texture4(program, 4);
				} else if (!Q_stricmp(token, "u_Texture5;")) {
					program->u_Texture5 = qglGetUniformLocation(program->program, "u_Texture5");
					R_GLSL_SetUniform_Texture5(program, 5);
				} else if (!Q_stricmp(token, "u_Texture6;")) {
					program->u_Texture6 = qglGetUniformLocation(program->program, "u_Texture6");
					R_GLSL_SetUniform_Texture6(program, 6);
				} else if (!Q_stricmp(token, "u_Texture7;")) {
					program->u_Texture7 = qglGetUniformLocation(program->program, "u_Texture7");
				...
				if (!Q_stricmp(token, "u_AmbientLight;")) {
					...
					program->u_DynamicLight = qglGetUniformLocation(program->program, "u_DynamicLight");
					...
					program->u_LightDirection = qglGetUniformLocation(program->program, "u_LightDirection");
				...
				if (!Q_stricmp(token, "u_ConstantColor;")) {
					program->u_ConstantColor = qglGetUniformLocation(program->program, "u_ConstantColor");
				} else if (!Q_stricmp(token, "u_EntityColor;")) {
					program->u_EntityColor = qglGetUniformLocation(program->program, "u_EntityColor");
				} else if (!Q_stricmp(token, "u_FogColor;")) {
					program->u_FogColor = qglGetUniformLocation(program->program, "u_FogColor");
				} else {
...
static qboolean R_GLSL_LoadProgram(glslProgram_t *program, const char *name, const char *programVertexObjects, int numVertexObjects, const char *programFragmentObjects, int numFragmentObjects) {
	GLchar		*buffer_vp[MAX_PROGRAM_OBJECTS];
	GLchar		*buffer_fp[MAX_PROGRAM_OBJECTS];
	GLchar		*buffer;
	...
	for (i = 0, str = (char *)programVertexObjects; i < numVertexObjects; i++, str += MAX_QPATH) {
		...
		shader_vp = qglCreateShader(GL_VERTEX_SHADER);
		qglShaderSource(shader_vp, 1, (const GLchar **)&buffer_vp, NULL);
		qglCompileShader(shader_vp);

		/* check for errors in vertex shader */
		qglTexParameteri(shader_vp, GL_OBJECT_COMPILE_STATUS_ARB, &status);
		if (!status) {
			int		length;
			char	*msg;

			/* print glsl error message */
			qglTexParameteri(shader_vp, GL_OBJECT_INFO_LOG_LENGTH_ARB, &length);
			msg = ri.Hunk_AllocateTempMemory(length);
			qglGetShaderInfoLog(shader_vp, length, &length, msg);
		...
		qglAttachShader(program->program, shader_vp);
		qglDeleteShader(shader_vp);
	}
	...
	for (i = 0, str = (char *)programFragmentObjects; i < numFragmentObjects; i++, str += MAX_QPATH) {
		...
		shader_fp = qglCreateShader(GL_FRAGMENT_SHADER);
		qglShaderSource(shader_fp, 1, (const GLchar **)&buffer_fp[i], NULL);
		qglCompileShader(shader_fp);

		/* check for errors in fragment shader */
		qglTexParameteri(shader_fp, GL_OBJECT_COMPILE_STATUS_ARB, &status);
		if (!status) {
			int		length;
			char	*msg;

			/* print glsl error message */
			qglTexParameteri(shader_fp, GL_OBJECT_INFO_LOG_LENGTH_ARB, &length);
			msg = ri.Hunk_AllocateTempMemory(length);
			qglGetShaderInfoLog(shader_fp, length, &length, msg);
			ri.Printf(PRINT_ALL, "Error:\n%s\n", msg);
			ri.Hunk_FreeTempMemory(msg);
		...
		qglAttachShader(program->program, shader_fp);
		qglDeleteShader(shader_fp);
	}
	...
	qglTexParameteri(program->program, GL_OBJECT_LINK_STATUS_ARB, &status);
	if (!status) {
		int		length;
		char	*msg;

		/* print glsl error message */
		qglTexParameteri(program->program, GL_OBJECT_INFO_LOG_LENGTH_ARB, &length);
		msg = ri.Hunk_AllocateTempMemory(length);
		qglGetShaderInfoLog(program->program, length, &length, msg);
		ri.Printf(PRINT_ALL, "Error:\n%s\n", msg);
		ri.Hunk_FreeTempMemory(msg);
...
qhandle_t RE_GLSL_RegisterProgram(const char *name, const char *programVertexObjects, int numVertexObjects, const char *programFragmentObjects, int numFragmentObjects) {
	...
	if (!R_GLSL_LoadProgram(program, name, programVertexObjects, numVertexObjects, programFragmentObjects, numFragmentObjects)) {
		qglDeleteShader(program->program);
