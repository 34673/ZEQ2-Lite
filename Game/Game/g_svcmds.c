...
static void UpdateIPBans(void){
	...
	*iplist_final = 0;
	for(i=0;i<numIPFilters;i++){
		if(ipFilters[i].compare == 0xffffffff){continue;}
		*(unsigned*)b = ipFilters[i].compare;
		*(unsigned*)m = ipFilters[i].mask;
		*ip = 0;
		for(j=0;j<4;j++){
			char* byte = m[j] != 255 ? "*" : va("%i",b[j]);
			Q_strcat(ip,sizeof(ip),byte);
...
static void AddIP(char* str){
	...
	for(i=0;i<numIPFilters;i++){
		// free spot
		if(ipFilters[i].compare == 0xffffffff){break;}
	}
	if(i == numIPFilters){
	...
	if(!StringToFilter(str,&ipFilters[i])){ipFilters[i].compare = 0xffffffffu;}
	UpdateIPBans();
}
...
void Svcmd_RemoveIP_f(void){
	...
	trap_Argv(1,str,sizeof(str));
	if(!StringToFilter(str,&f)){return;}
	for(i=0;i<numIPFilters;i++){
