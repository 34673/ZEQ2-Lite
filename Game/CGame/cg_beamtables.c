...
static void CG_BezierMidPoints(const vec3_t startPos, const vec3_t endPos,
	...
	tangentScale = .5f * Distance(startPos, endPos);
...
static int CG_TessCount(const beamTableElem_t *startElem, const beamTableElem_t *endElem){
	float	directTangentRatio, indirectTangentRatio,
			tangentRatio, dist;
	vec3_t	dir;
	int		detail;
	...
	if((dist = VectorNormalize(dir)) == 0) return 0;
	...
	if(r_beamDetail.value >= 4){
		if(tangentRatio < 1) tangentRatio = 1;
		detail = 40;
	}
	else if(r_beamDetail.value == 3){
		if(tangentRatio < .5f) tangentRatio = .5f;
		detail = 30;
	}
	else if(r_beamDetail.value == 2){
		if(tangentRatio < .25f) tangentRatio = .25f;
		detail = 20;
	}
	else if(r_beamDetail.value == 1){
		if(tangentRatio < .1f) tangentRatio = .1f;
		detail = 10;
	}
...
static void CG_DrawBeamTable (int clientNum, qboolean alternate){
	beamTable_t			*currentTable;
	beamTableElem_t		*currentElem, *prevElem, starter;
	polyVert_t			verts[4];
	vec3_t				midPos1, midPos2,
						tessPoint, tessTangent;
	int					tessSize, i, j;
	float				t;
	...
	while(1){
		...
		CG_BezierMidPoints(prevElem->pos, currentElem->pos, prevElem->tangent,
							currentElem->tangent, midPos1, midPos2);
