...
The "base game" is the directory under the paths where data comes from by default ("ZEQ2").
...
(such as ZEQ2 + <mod> content combination in a mod for instance)
BASEGAME is the hardcoded base game ("ZEQ2")
...
typedef struct {
	...
	char			pakGamename[MAX_OSPATH];	// ZEQ2
...
typedef struct {
	char		path[MAX_OSPATH];		// Build path
	char		fullpath[MAX_OSPATH];	// path + gamedir
	char		gamedir[MAX_OSPATH];	// ZEQ2
} directory_t;
...
A mod directory is a peer to ZEQ2 with a pk3 or pk3dir in it
================
*/
...
const char *FS_ReferencedPakNames( void ) {
	...
	// and referenced one's from ZEQ2
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		// is the element a pak file?
		if ( search->pack ) {
			if (search->pack->referenced || Q_stricmpn(search->pack->pakGamename, com_basegame->string, strlen(com_basegame->string))) {
