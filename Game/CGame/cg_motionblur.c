...
void CG_MotionBlur(void){
	...
	if(!(cg.snap->ps.bitFlags & usingBoost) && cg.snap->ps.timers[tmZanzoken] < 1 && !cg.snap->ps.timers[tmTransform]){
		...
		/*
		for(i=0; i<MAX_MOTIONBLURDOTS; i++) cg_motionblurDots[i].active = qfalse;
		*/
		return;
	}
	cg.refdef.rdflags |= RDF_MOTIONBLUR;
	...
	for(i=0; i<MAX_MOTIONBLURDOTS; i++){
		...
		if(dot->lifeTime + dot->startTime < cg.time) dot->active = qfalse;
	}
	// Create new dots
	for(i=0; i<MAX_MOTIONBLURDOTS; i++){
		dot = &cg_motionblurDots[i];
		...
		dot->refEnt.shaderRGBA[0] = dot->refEnt.shaderRGBA[1] = dot->refEnt.shaderRGBA[2] = 255;
	...
	for(i=0; i<MAX_MOTIONBLURDOTS; i++){
		dot = &cg_motionblurDots[i];
		if(dot->startTime > cg.time) continue;
