...
static void CG_DrawBoxFace(vec3_t a, vec3_t b, vec3_t c, vec3_t d){
	polyVert_t	verts[4];
	vec4_t		color = {255.f, 0, 0, 128.f };
	qhandle_t	bboxShader;

	VectorCopy(d, verts[0].xyz);
	verts[0].st[0] = verts[0].st[1] = 1;
...
void CG_SetEntitySoundPosition(centity_t *cent){
	...
	else trap_S_UpdateEntityPosition(cent->currentState.number, cent->lerpOrigin);
}
...
static void CG_EntityEffects(centity_t *cent){
	...
	if(cent->currentState.loopSound){
		...
		else trap_S_AddRealLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, 
									cgs.gameSounds[cent->currentState.loopSound]);
	}
	// constant light glow
...
static void CG_General(centity_t *cent){
	...
	if(s1->number == cg.snap->ps.clientNum) ent.renderfx |= RF_THIRD_PERSON;	
...
static void CG_Speaker(centity_t *cent){
	...
	// note: client num check not auto triggering
	if(!cent->currentState.clientNum || cg.time < cent->miscTime) return;
...
void CG_TrailFunc_SpiralBeam_Helper(entityState_t *es, int time, vec3_t origin){
	...
	if(!VectorNormalize2(es->pos.trDelta, tmpAxis[0])) tmpAxis[0][2] = 1;
...
void CG_TrailFunc_SpiralBeam( centity_t *ent ) {
	...
	es = &ent->currentState;
	weaponGraphics = CG_FindUserWeaponGraphics(es->clientNum, es->weapon);
	if(!weaponGraphics->missileTrailSpiralShader) return;
...
void CG_TrailFunc_FadeTail( centity_t *cent ) {
	...
	weaponGraphics = CG_FindUserWeaponGraphics(es->clientNum, es->weapon);	
	if(!weaponGraphics->missileTrailShader || !weaponGraphics->missileTrailRadius) return;
	...
	// -200 gives .2 sec leeway, just in case
	if(cent->lastTrailTime < (cg.time -cg.frametime -200)){
		BG_EvaluateTrajectoryDelta(es, &es->pos, cg.time, delta);
		CG_ResetTrail(es->number, cent->lerpOrigin, VectorLength(delta), weaponGraphics->missileTrailRadius,
						weaponGraphics->missileTrailShader, NULL);
	}
...
static void CG_Missile(centity_t *cent){
	...
	if(!(weaponGraphics->missileModel && weaponGraphics->missileSkin)){
		...
		//ent.shaderRGBA[0] = ent.shaderRGBA[1] = ent.shaderRGBA[2] = ent.shaderRGBA[3] = radiusScale;
		trap_R_AddRefEntityToScene(&ent);
		CG_PlayerSplash(cent, 500 / (missileScale / 4));
	}
	else{
		ent.reType = RT_MODEL;
		if((weaponGraphics->missileStruggleModel && weaponGraphics->missileStruggleSkin) && missileIsStruggling/*(s1->dashDir[2] != 0)*/){
		...
		if(!weaponGraphics->missileSpin[0] && !weaponGraphics->missileSpin[1]){					
			// convert direction of travel into axis
			if(!VectorNormalize2(s1->pos.trDelta, ent.axis[0]))
				ent.axis[0][2] = 1;
			// spin as it moves
			if(s1->pos.trType != TR_STATIONARY)
				RotateAroundDirection(ent.axis, weaponGraphics->missileSpin[2] *cg.time / 4.f);
			else
				RotateAroundDirection(ent.axis, weaponGraphics->missileSpin[2] *s1->time);
		}
		else{
			vec3_t tempAngles, spinRotate;
			vec4_t quatOrient, quatRot, quatResult;

			// convert direction of travel into axis
			if(!VectorNormalize2(s1->pos.trDelta, ent.axis[0]))
				ent.axis[0][0] = 1;
			if(s1->pos.trType != TR_STATIONARY)
				VectorSet(spinRotate, cg.time / 4.f, cg.time / 4.f, cg.time / 4.f);
			else
	...
	if(!CG_FrameHist_WasInPVS(s1->number))
		if(weaponGraphics->missileParticleSystem[0]){
			...
			PSys_SpawnCachedSystem(weaponGraphics->missileParticleSystem, cent->lerpOrigin,
									tempAxis, cent, NULL, qfalse, qfalse);
		}
}
...
/*static*/ void CG_Mover(centity_t *cent){
	...
	else ent.hModel = cgs.gameModels[s1->modelindex];
	// add to refresh list
	trap_R_AddRefEntityToScene(&ent);
...
static void CG_Portal(centity_t *cent){
	...
	ent.skinNum =	s1->clientNum / 256.f *360;	// roll offset
	// add to refresh list
	trap_R_AddRefEntityToScene(&ent);
}
...
void CG_AddPacketEntities(void){
	...
	if(cg.nextSnap){
		...
		if(!delta)	cg.frameInterpolation = 0;
		else		cg.frameInterpolation = (float)(cg.time -cg.snap->serverTime) / delta;
	}
	// actually, it should never be used, because 
	// no entities should be marked as interpolating
	else cg.frameInterpolation = 0;	
	// the auto-rotating items will all have the same axis
	cg.autoAngles[1] = (cg.time & 2047) *360 / 2048.f;
	cg.autoAngles[0] = cg.autoAngles[2] = 0;
	cg.autoAnglesFast[1] = (cg.time & 1023) *360 / 1024.f;
	cg.autoAnglesFast[0] = cg.autoAnglesFast[2] = 0;
