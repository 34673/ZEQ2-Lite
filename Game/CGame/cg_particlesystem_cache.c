		...
		if(!token[0]){
			...
			if(!token[0]){
				...
				return qfalse;
				 ...
				 if(!Q_stricmp(token, "regular")) cachePtcl->rType = RTYPE_DEFAULT;
			else if(!Q_stricmp(token, "spark")) cachePtcl->rType = RTYPE_SPARK;
			else if(!Q_stricmp(token, "ray")) cachePtcl->rType = RTYPE_RAY;
			else{
			...
			if(!PSys_ParseVector(text_pp, 4, cachePtcl->rgba.midVal, qtrue)) return qfalse;
			...
			if(!PSys_ParseVector( text_pp, 4, cachePtcl->rgba.startVal, qtrue )) return qfalse;
			...
			if(!PSys_ParseVector( text_pp, 4, cachePtcl->rgba.endVal, qtrue )) return qfalse;
			...
			if(!PSys_ParseVector( text_pp, 4, cachePtcl->rotate.midVal, qtrue )) return qfalse;
			...
			if(!PSys_ParseVector( text_pp, 4, cachePtcl->rotate.startVal, qtrue )) return qfalse;
			...
			if(!PSys_ParseVector(text_pp, 4, cachePtcl->rotate.endVal, qtrue)) return qfalse;
		...
		if(!token[0]){
			...
			if(!token[0]){
				...
				return qfalse;
				 ...
				 if(!Q_stricmp(token, "point")) cacheEmit->type = ETYPE_POINT;
			else if(!Q_stricmp(token, "radius")) cacheEmit->type = ETYPE_RADIUS;
			else if(!Q_stricmp(token, "sphere")) cacheEmit->type = ETYPE_SPHERE;
			else if(!Q_stricmp(token, "pointGround")) cacheEmit->type = ETYPE_POINT_SURFACE;
			else if(!Q_stricmp(token, "radiusGround")) cacheEmit->type = ETYPE_RADIUS_SURFACE;
			else{
			...
			if(!PSys_ParseVector(text_pp, 3, cacheEmit->orientation.offset, qfalse)) return qfalse;
		}
			...
			if(!PSys_ParseVector(text_pp, 3, cacheEmit->orientation.dir, qfalse)) return qfalse;
		}
		else if(!Q_stricmp(token, "}")){
			if(ptclKey) CG_Printf(S_COLOR_YELLOW "WARNING: empty particle declaration\n");
			return qtrue;
		}
		else{
			CG_Printf(S_COLOR_RED "ERROR: '%s': not a valid keyword\n", token);
		...
		if(!token[0]){
			...
			if(!token[0]){
				...
				return qfalse;
				 ...
				 if(!Q_stricmp(token, "directional")) cacheForce->type = FTYPE_DIRECTIONAL;
			else if(!Q_stricmp(token, "spherical")) cacheForce->type = FTYPE_SPHERICAL;
			else if(!Q_stricmp(token, "drag")) cacheForce->type = FTYPE_DRAG;
			else if(!Q_stricmp(token, "spin")) cacheForce->type = FTYPE_SWIRL;
			else{
			...
			if(!token[0]){
				...
				return qfalse;
				 ...
				 if(!Q_stricmp(token, "infinite")) cacheForce->AOItype = AOI_INFINITE;
		...
		if(!token[0]){
			...
			if(!token[0]){
				...
				return qfalse;
				 ...
				 if(!Q_stricmp(token, "plane")) cacheConst->type = CTYPE_PLANE;
			else if(!Q_stricmp(token, "maxDistance")) cacheConst->type = CTYPE_DISTANCE_MAX;
			else if(!Q_stricmp(token, "minDistance")) cacheConst->type = CTYPE_DISTANCE_MIN;
			else if(!Q_stricmp(token, "exactDistance")) cacheConst->type = CTYPE_DISTANCE_EXACT;
			else{
...
static qboolean PSys_ParseSystem(char **text_pp, PSys_SystemTemplate_t *cacheSys){
	...
	while(1){
		...
		if(!token[0]){
			...
			if(memberType != MEM_NONE) CG_Printf(S_COLOR_YELLOW "WARNING: empty member declaration\n");
			return qtrue;
		}
		else{
			CG_Printf(S_COLOR_RED "ERROR: '%s': not a valid keyword\n", token);
...
PSys_SystemTemplate_t* PSys_LoadSystemFromCache(char *systemName){
	...
	if(!*systemName) return NULL;
	// Zero systems cached
	if(!PSys_CurCacheSize) return NULL;
...
void PSys_InitCache(void){
	...
	if(PSys_CurCacheSize > 1) PSys_QSortCache(0, PSys_CurCacheSize -1);
