...
qboolean G_SpawnFloat(const char* key,const char* defaultString,float* out){
	char* s;
	qboolean present = G_SpawnString(key,defaultString,&s);
	*out = atof(s);
	return present;
}
...
void G_SpawnGEntityFromSpawnVars(void){
	...
	char* value;
	char* gametypeName;
	static char* gametypeNames[] = {"ffa","single"};
...
qboolean G_ParseSpawnVars(void){
	...
	// end of spawn string
	if(!trap_GetEntityToken(com_token,sizeof(com_token))){return qfalse;}
	if(com_token[0] != '{'){
...
void SP_worldspawn(void){
	...
	trap_SetConfigstring(CS_PRODUCT_VERSION,PRODUCT_VERSION);
...
void G_SpawnEntitiesFromString(void){
	...
	SP_worldspawn();
	// parse ents
	while(G_ParseSpawnVars()){G_SpawnGEntityFromSpawnVars();}
