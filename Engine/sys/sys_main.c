...
=================
*/
static char *Sys_PIDFileName( const char *dir )
{
	const char *homePath = Cvar_VariableString( "fs_homePath" );

	if( *homePath != '\0' )
		return va( "%s/%s/%s", homePath, dir,PID_FILENAME );
...
=================
*/
static qboolean Sys_WritePIDFile( const char *dir )
{
	char      *pidFile = Sys_PIDFileName( dir );
	FILE      *f;
...
void Sys_InitPIDFile( const char *dir ) {
	if( Sys_WritePIDFile( dir ) ) {
#ifndef DEDICATED
		char message[1024];
		char modName[MAX_OSPATH];

		FS_GetModDescription( dir, modName, sizeof ( modName ) );
		Q_CleanStr( modName );
...
static __attribute__ ((noreturn)) void Sys_Exit( int exitCode )
{
	...
	{
		// Normal exit
		Sys_RemovePIDFile( FS_GetCurrentDir() );
	}
