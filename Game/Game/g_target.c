...
void SP_target_remove_powerups(gentity_t* ent){ent->use = Use_target_remove_powerups;}
...
void SP_target_delay(gentity_t* ent){
	...
	if(!G_SpawnFloat("delay","0",&ent->wait)){G_SpawnFloat("wait","1", &ent->wait);}
...
void Use_Target_Speaker(gentity_t* ent,gentity_t* other,gentity_t* activator){
	...
	if(ent->spawnflags & 3){
		ent->s.loopSound = ent->s.loopSound ? 0 : ent->noise_index;
	}
	// normal sound
	else{
		if(ent->spawnflags & 8){
...
void target_laser_use(gentity_t* self,gentity_t* other,gentity_t* activator){
	...
	if(self->nextthink > 0){target_laser_off(self);}
...
void target_laser_start(gentity_t* self){
	...
	if(self->target){
		...
		if(!ent){G_Printf("%s at %s: %s is a bad target\n",self->classname,vtos(self->s.origin),self->target);}
		self->enemy = ent;
	}
	else{
...
void target_relay_use(gentity_t* self,gentity_t* other,gentity_t* activator){
	if(self->spawnflags & 4){
		...
		if(ent && ent->use){ent->use(ent,self,activator);}
		return;
	}
...
static void target_location_linkup(gentity_t* ent){
	...
	for(ent=g_entities;i<level.num_entities;i++,ent++){
		if(ent->classname && !Q_stricmp(ent->classname,"target_location")){
			// lets overload some variables!
			// use for location marking
			ent->powerLevelTotal = n;
