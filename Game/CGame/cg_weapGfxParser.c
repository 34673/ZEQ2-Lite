...
// FIXME: Can this be a local variable instead, or would it give us > 32k locals errors in the VM-bytecode compiler?
/*
...
qboolean CG_weapGfx_ParseModel(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
		case CAT_CHARGE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.chargeModel, "", sizeof(cg_weapGfxBuffer.chargeModel));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.chargeModel, token->stringval, sizeof(cg_weapGfxBuffer.chargeModel));}
			else{
			...
			break;
		case CAT_EXPLOSION:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.explosionModel, "", sizeof(cg_weapGfxBuffer.explosionModel));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.explosionModel, token->stringval, sizeof(cg_weapGfxBuffer.explosionModel));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_STRUGGLE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileStruggleModel, "", sizeof(cg_weapGfxBuffer.missileStruggleModel));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileStruggleModel, token->stringval, sizeof(cg_weapGfxBuffer.missileStruggleModel));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_MISSILE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileModel, "", sizeof(cg_weapGfxBuffer.missileModel));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileModel, token->stringval, sizeof(cg_weapGfxBuffer.missileModel));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.flashModel, "", sizeof(cg_weapGfxBuffer.flashModel));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.flashModel, token->stringval, sizeof(cg_weapGfxBuffer.flashModel));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
	...
	if(!CG_weapGfx_NextSym(scanner, token)){
		if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
		return qfalse;
	}
	return qtrue;
}
...
qboolean CG_weapGfx_ParseSkin(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
		case CAT_CHARGE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.chargeSkin, "", sizeof(cg_weapGfxBuffer.chargeSkin));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.chargeSkin, token->stringval, sizeof(cg_weapGfxBuffer.chargeSkin));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_EXPLOSION:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.explosionSkin, "", sizeof(cg_weapGfxBuffer.explosionSkin));
			}else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.explosionSkin, token->stringval, sizeof(cg_weapGfxBuffer.explosionSkin));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_STRUGGLE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileStruggleSkin, "", sizeof(cg_weapGfxBuffer.missileStruggleSkin));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz( cg_weapGfxBuffer.missileStruggleSkin, token->stringval, sizeof(cg_weapGfxBuffer.missileStruggleSkin));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_MISSILE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileSkin, "", sizeof(cg_weapGfxBuffer.missileSkin));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileSkin, token->stringval, sizeof(cg_weapGfxBuffer.missileSkin));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.flashSkin, "", sizeof(cg_weapGfxBuffer.flashSkin));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.flashSkin, token->stringval, sizeof(cg_weapGfxBuffer.flashSkin));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
...
qboolean CG_weapGfx_ParseShader(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
		case CAT_CHARGE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.chargeShader, "", sizeof(cg_weapGfxBuffer.chargeShader));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.chargeShader, token->stringval, sizeof(cg_weapGfxBuffer.chargeShader));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_EXPLOSION:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.explosionShader, "", sizeof(cg_weapGfxBuffer.explosionShader));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.explosionShader, token->stringval, sizeof(cg_weapGfxBuffer.explosionShader));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_STRUGGLE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileStruggleShader, "", sizeof(cg_weapGfxBuffer.missileStruggleShader));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileStruggleShader, token->stringval, sizeof(cg_weapGfxBuffer.missileStruggleShader));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_MISSILE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileShader, "", sizeof(cg_weapGfxBuffer.missileShader));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileShader, token->stringval, sizeof(cg_weapGfxBuffer.missileShader));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.flashShader, "", sizeof(cg_weapGfxBuffer.flashShader));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.flashShader, token->stringval, sizeof(cg_weapGfxBuffer.flashShader));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_TRAIL:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz( cg_weapGfxBuffer.missileTrailShader, "", sizeof(cg_weapGfxBuffer.missileTrailShader));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileTrailShader, token->stringval, sizeof(cg_weapGfxBuffer.missileTrailShader));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
	...
	switch(category){
		case CAT_CHARGE:
			if((token->tokenSym == TOKEN_INTEGER) || (token->tokenSym == TOKEN_FLOAT)){
				...
				else{cg_weapGfxBuffer.chargeStartsize = token->floatval;}
				if(!CG_weapGfx_NextSym(scanner, token)){
					if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
					return qfalse;
				}
				...
				else{cg_weapGfxBuffer.chargeEndsize = token->floatval;}
				if(!CG_weapGfx_NextSym(scanner, token)){
					if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
					return qfalse;
				}
		...
		case CAT_EXPLOSION:
			...
			cg_weapGfxBuffer.explosionSize = token->floatval; 
			break;
		case CAT_MISSILE:
			if((token->tokenSym != TOKEN_INTEGER) && (token->tokenSym != TOKEN_FLOAT)){
				CG_weapGfx_ErrorHandle(ERROR_FLOAT_EXPECTED, scanner, token->stringval, NULL);
			...
			cg_weapGfxBuffer.missileSize = token->floatval;
			break;
		case CAT_TRAIL:
			if((token->tokenSym != TOKEN_INTEGER) && (token->tokenSym != TOKEN_FLOAT)){
				CG_weapGfx_ErrorHandle(ERROR_FLOAT_EXPECTED, scanner, token->stringval, NULL);
			...
			cg_weapGfxBuffer.missileTrailRadius = token->floatval;
			break;
		case CAT_FLASH:
			if((token->tokenSym != TOKEN_INTEGER) && (token->tokenSym != TOKEN_FLOAT)){
				CG_weapGfx_ErrorHandle(ERROR_FLOAT_EXPECTED, scanner, token->stringval, NULL);
			...
			cg_weapGfxBuffer.flashSize = token->floatval;
			break;
		default:
			CG_weapGfx_ErrorHandle(ERROR_FIELD_NOT_IN_CATEGORY, scanner, cg_weapGfxFields[field].fieldname, cg_weapGfxCategories[category]);
			return qfalse;
...
qboolean CG_weapGfx_ParseDlight(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if((token->tokenSym != TOKEN_INTEGER) && (token->tokenSym != TOKEN_FLOAT)){
		...
		if(token->floatval > 1.f){
			CG_weapGfx_ErrorHandle(ERROR_OVER_MAXBOUND, scanner, token->stringval, "1.0");
			return qfalse;
		}
		else if(token->floatval < 0.f){CG_weapGfx_ErrorHandle(ERROR_UNDER_MINBOUND, scanner, token->stringval, "0.0");}
		else{RGB[0] = token->floatval;}
	}
	if(!CG_weapGfx_NextSym(scanner, token)){
		if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
		return qfalse;
	...
	switch(category){
		case CAT_CHARGE:
			if((token->tokenSym == TOKEN_INTEGER) || (token->tokenSym == TOKEN_FLOAT)){
				...
				else{cg_weapGfxBuffer.chargeDlightStartRadius = token->floatval;}
				if(!CG_weapGfx_NextSym(scanner, token)){
					if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
					return qfalse;
				}
				...
				else{cg_weapGfxBuffer.chargeDlightEndRadius = token->floatval;}
				if(!CG_weapGfx_NextSym(scanner, token)){
					if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
					return qfalse;
				}
...
qboolean CG_weapGfx_ParseTagTo(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	else{Q_strncpyz(cg_weapGfxBuffer.chargeTag[0], token->stringval, sizeof(cg_weapGfxBuffer.chargeTag[0]));}
	if(!CG_weapGfx_NextSym(scanner, token)){
		if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle( ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseSoundFx( cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field ) {
	...
	switch(category){
		case CAT_EXPLOSION:
			//4 samples max
			if(token->tokenSym == TOKEN_NULL){memset(cg_weapGfxBuffer.explosionSound, 0, sizeof(char)*4*MAX_QPATH);}
			...
			break;
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){memset(cg_weapGfxBuffer.flashSound, 0, sizeof(char)*4*MAX_QPATH);}
			else if(token->tokenSym == TOKEN_STRING){
...
qboolean CG_weapGfx_ParseVoiceFx(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){memset(cg_weapGfxBuffer.voiceSound, 0, sizeof(char)*4*MAX_QPATH);}
			else if( token->tokenSym == TOKEN_STRING){
...
qboolean CG_weapGfx_ParseLoopFx(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
		case CAT_CHARGE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.chargeLoopSound, "", sizeof(cg_weapGfxBuffer.chargeLoopSound));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.chargeLoopSound, token->stringval, sizeof(cg_weapGfxBuffer.chargeLoopSound));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_MISSILE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileSound, "", sizeof(cg_weapGfxBuffer.missileSound));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileSound, token->stringval, sizeof(cg_weapGfxBuffer.missileSound));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.firingSound, "", sizeof(cg_weapGfxBuffer.firingSound));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.firingSound, token->stringval, sizeof(cg_weapGfxBuffer.firingSound));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
...
qboolean CG_weapGfx_ParseTimedFx(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	while(token->tokenSym == TOKEN_INTEGER){
		...
		else{cg_weapGfxBuffer.chargeVoice[i].startPct = token->intval;}
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
			return qfalse;
		}
		...
		else{Q_strncpyz(cg_weapGfxBuffer.chargeVoice[i].voice, token->stringval, sizeof(cg_weapGfxBuffer.chargeVoice[i].voice));}
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
			return qfalse;
		}
...
qboolean CG_weapGfx_ParseOnceFx(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.flashOnceSound, "", sizeof(cg_weapGfxBuffer.flashOnceSound));}
	else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.flashOnceSound, token->stringval, sizeof(cg_weapGfxBuffer.flashOnceSound));}
	else{
		CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}		
...
qboolean CG_weapGfx_ParseDuration(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	else{cg_weapGfxBuffer.explosionTime = token->intval;}
	if(!CG_weapGfx_NextSym(scanner, token)){
		if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseShockwave(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){
		...
		if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.shockwaveSkin, token->stringval, sizeof(cg_weapGfxBuffer.shockwaveSkin));}
		else{
			CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
			return qfalse;
		}
...
qboolean CG_weapGfx_ParseMarkShader(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.markShader, "", sizeof(cg_weapGfxBuffer.markShader));}
	else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.markShader, token->stringval, sizeof(cg_weapGfxBuffer.markShader));}
	else{
		CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseSmokeParticles(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.smokeParticleSystem, "", sizeof(cg_weapGfxBuffer.smokeParticleSystem));}
	else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.smokeParticleSystem, token->stringval, sizeof(cg_weapGfxBuffer.smokeParticleSystem));}
	else{
		CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseLoopParticles(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
	case CAT_CHARGE:
		if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.chargeParticleSystem, "", sizeof(cg_weapGfxBuffer.chargeParticleSystem));}
		else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.chargeParticleSystem, token->stringval, sizeof(cg_weapGfxBuffer.chargeParticleSystem));}
		else{
			CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
			return qfalse;
		}
		break;
	case CAT_FLASH:
		if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.firingParticleSystem, "", sizeof(cg_weapGfxBuffer.firingParticleSystem));}
		else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.firingParticleSystem, token->stringval, sizeof(cg_weapGfxBuffer.firingParticleSystem));}
		else{
			CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
			return qfalse;
		}		
...
qboolean CG_weapGfx_ParseParticles(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	switch(category){
		case CAT_EXPLOSION:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.explosionParticleSystem, "", sizeof(cg_weapGfxBuffer.explosionParticleSystem));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.explosionParticleSystem, token->stringval, sizeof(cg_weapGfxBuffer.explosionParticleSystem));}
			else{CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}		
			break;
		case CAT_MISSILE:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileParticleSystem, "", sizeof(cg_weapGfxBuffer.missileParticleSystem));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileParticleSystem, token->stringval, sizeof(cg_weapGfxBuffer.missileParticleSystem));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			break;
		case CAT_FLASH:
			if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.flashParticleSystem, "", sizeof(cg_weapGfxBuffer.flashParticleSystem));}
			else if(token->tokenSym == TOKEN_STRING){Q_strncpyz( cg_weapGfxBuffer.flashParticleSystem, token->stringval, sizeof(cg_weapGfxBuffer.flashParticleSystem));}
			else{
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
...
qboolean CG_weapGfx_ParseSpiralShader(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.missileTrailSpiralShader, "", sizeof(cg_weapGfxBuffer.missileTrailSpiralShader));}
	else if(token->tokenSym == TOKEN_STRING){Q_strncpyz(cg_weapGfxBuffer.missileTrailSpiralShader, token->stringval, sizeof(cg_weapGfxBuffer.missileTrailSpiralShader));}
	else{
		CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseSpiralSize(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if((token->tokenSym == TOKEN_INTEGER) || (token->tokenSym == TOKEN_FLOAT)){cg_weapGfxBuffer.missileTrailSpiralRadius = token->floatval;}
	else{
		CG_weapGfx_ErrorHandle(ERROR_INTEGER_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseSpiralOffset(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if((token->tokenSym == TOKEN_INTEGER) || (token->tokenSym == TOKEN_FLOAT)){cg_weapGfxBuffer.missileTrailSpiralOffset = token->floatval;}
	else{
		CG_weapGfx_ErrorHandle(ERROR_INTEGER_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseIcon(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.weaponIcon, "", sizeof(cg_weapGfxBuffer.missileTrailSpiralShader));}
	else if(token->tokenSym == TOKEN_STRING){Q_strncpyz( cg_weapGfxBuffer.weaponIcon, token->stringval, sizeof(cg_weapGfxBuffer.missileTrailSpiralShader));}
	else{
		CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
qboolean CG_weapGfx_ParseDisplayName(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category, int field){
	...
	if(token->tokenSym == TOKEN_NULL){Q_strncpyz(cg_weapGfxBuffer.weaponName, "", sizeof(cg_weapGfxBuffer.missileTrailSpiralShader));}
	else if(token->tokenSym == TOKEN_STRING ){Q_strncpyz(cg_weapGfxBuffer.weaponName, token->stringval, sizeof(cg_weapGfxBuffer.missileTrailSpiralShader));}
	else{
		CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
		return qfalse;
	}
...
static qboolean CG_weapGfx_ParseImports(cg_weapGfxParser_t *parser){
	...
	while(token->tokenSym == TOKEN_IMPORT){
		...
		else{Q_strncpyz(refname, token->stringval, sizeof(refname));}
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
			return qfalse;
		}
		...
		else{Q_strncpyz(filename, token->stringval, sizeof(filename));}
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
			return qfalse;
		}
		...
		if(!CG_weapGfx_AddImportRef(parser, refname, filename, token->stringval)){return qfalse;}
		// NOTE: Do it like this to prevent errors if a file happens to only contain import lines.
		// While it is actually useless to have such a file, it still is syntacticly correct.
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym != TOKEN_EOF){return qfalse;}
		}
	}
	return qtrue;
}
...
static qboolean CG_weapGfx_ParseFields(cg_weapGfxParser_t *parser, cg_weapGfxCategoryIndex_t category){
	...
	while(token->tokenSym == TOKEN_FIELD){
		...
		if(!cg_weapGfxFields[field].parseFunc(parser, category, field)){return qfalse;}		
	}
	return qtrue;
}
/*
...
static qboolean CG_weapGfx_ParseCategories(cg_weapGfxParser_t *parser){
	...
	while(token->tokenSym == TOKEN_CATEGORY){
		...
		// Handles fields
		if (!CG_weapGfx_ParseFields(parser, currentCategory)){return qfalse;}
	...
	if(cg_verboseParse.integer){CG_Printf("Processed categories succesfully.\n");}
	return qtrue;
}
/*
Pre-parses the definitions of the weapons in the scriptfile.
The contents of a definition block are checked for validity by the lexical scanner and parser to make sure syntax is correct.
The parsed values however, are not yet stored, because we don't have a link to an actual weapon yet.
Entry points into the definitions are cached into a table for a quick jump once we find out the links in CG_weapGfx_ParseLinks.
*/
static qboolean CG_weapGfx_PreParseDefinitions(cg_weapGfxParser_t *parser){
	...
	while((token->tokenSym == TOKEN_PRIVATE) || (token->tokenSym == TOKEN_PUBLIC) || (token->tokenSym == TOKEN_PROTECTED)){
		...
		else if(token->tokenSym == TOKEN_PROTECTED){accessLvl = LVL_PROTECTED;}
		else{accessLvl = LVL_PRIVATE;}
		hasSuper = qfalse;
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
			return qfalse;
		...
		if(token->tokenSym == TOKEN_EQUALS){
			...
			if(!CG_weapGfx_FindDefinitionRef(parser, token->stringval)){return qfalse;}
			else{Q_strncpyz(supername, token->stringval, sizeof(supername));}
			if(!CG_weapGfx_NextSym(scanner, token)){
				if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
				return qfalse;
			}
		...
		if(token->tokenSym != TOKEN_OPENBLOCK){
			...
			defpos = scanner->pos;
			if(!CG_weapGfx_AddDefinitionRef(parser, refname, defpos, defline, accessLvl, hasSuper, supername)){return qfalse;}
		}
		while(blockCount > 0){
			if(!CG_weapGfx_NextSym(scanner, token)){
				if(token->tokenSym == TOKEN_EOF){CG_weapGfx_ErrorHandle(ERROR_PREMATURE_EOF, scanner, NULL, NULL);}
		...
		// lines, but instead terminates after the definitions. This is what one would expect of 'library' files.
		if(!CG_weapGfx_NextSym(scanner, token)){
			if(token->tokenSym != TOKEN_EOF){return qfalse;}
		}
	}
...
static qboolean CG_weapGfx_ParseLinks(cg_weapGfxParser_t *parser){
	...
	while(token->tokenSym == TOKEN_WEAPON){
		...
		else{Q_strncpyz(pri_refname, token->stringval, sizeof(pri_refname));}
		...
		if(token->tokenSym == TOKEN_COLON){
			if(!CG_weapGfx_NextSym(scanner, token)){return qfalse;}
			if(token->tokenSym != TOKEN_STRING){
				CG_weapGfx_ErrorHandle(ERROR_STRING_EXPECTED, scanner, token->stringval, NULL);
				return qfalse;
			}
			...
			else{Q_strncpyz( sec_refname, token->stringval, sizeof(sec_refname));}
			// NOTE: This makes sure we don't get an error if this is the last link in the file.
			if(!CG_weapGfx_NextSym( scanner, token)){
				if(token->tokenSym != TOKEN_EOF){return qfalse;}
			}
		}
		else{Q_strncpyz(sec_refname, "", sizeof(sec_refname));}
		if(!CG_weapGfx_AddLinkRef(parser, weaponNum, pri_refname, sec_refname)){return qfalse;}
	}
	return qtrue;
}
/*
Increases inheritance recursion depth.
Checks if the maximum level is exceeded and returns an error if so.
*/
static qboolean CG_weapGfx_IncreaseRecursionDepth(void){
	if(cg_weapGfxRecursionDepth == MAX_RECURSION_DEPTH){return qfalse;}
...
qboolean CG_weapGfx_ParseRemoteDefinition( char *filename, char *refname ) {
	...
	// Get the very first token initialized. If it is an end of file token, we will not parse
	...
	if(i < MAX_DEFINES){
		if(parser.definitionRef[i].accessLvl != LVL_PUBLIC){
			//Make sure the error reports the correct line
	...
	if(!CG_weapGfx_ParseDefinition(&parser, refname, NULL)){return qfalse;}
	return qtrue;
}
/*
============================
...
qboolean CG_weapGfx_ParseDefinition(cg_weapGfxParser_t *parser, char* refname, cg_weapGfxAccessLvls_t *accessLvl){
	...
	if(i < MAX_DEFINES){
		...
		if(parser->definitionRef[i].hasSuper){
			...
			if(cg_verboseParse.integer){CG_Printf("Inheriting superclass '%s'\n", parser->definitionRef[i].supername);}
			if(!CG_weapGfx_ParseDefinition(parser, parser->definitionRef[i].supername, &lastAccessLvl)){return qfalse;}
		...
		if(cg_verboseParse.integer){CG_Printf("Importing '%s'\n", refname);}
		if(!CG_weapGfx_ParseRemoteDefinition(parser->importRef[i].filename, parser->importRef[i].defname)){return qfalse;}
		CG_weapGfx_DecreaseRecursionDepth();
	}
	return qtrue;
}
...
qboolean CG_weapGfx_Parse(char *filename, int clientNum){
	...
	// Work through the link table, parsing and assigning the definitions as we go.
	for(i=0;i < MAX_LINKS;i++){
		if(!parser.linkRef[i].active){continue;}
		// Empty the buffer.
		memset(&cg_weapGfxBuffer, 0, sizeof(cg_weapGfxBuffer));
		if(cg_verboseParse.integer){CG_Printf("Processing weapon nr %i, primary '%s'.\n", i+1, parser.linkRef[i].pri_refname);}
		if(!CG_weapGfx_ParseDefinition(&parser, parser.linkRef[i].pri_refname, NULL)){return qfalse;}
		CG_weapGfx_StoreBuffer(clientNum, i);
		memset(&cg_weapGfxBuffer, 0, sizeof(cg_weapGfxBuffer));
		if(strcmp(parser.linkRef[i].sec_refname, "")){
			if(cg_verboseParse.integer){CG_Printf("Processing weapon nr %i, secondary '%s'.\n", i+1, parser.linkRef[i].sec_refname);}
			if(!CG_weapGfx_ParseDefinition(&parser, parser.linkRef[i].sec_refname, NULL)){return qfalse;}
		}
		CG_weapGfx_StoreBuffer(clientNum, i + ALTWEAPON_OFFSET);
	}
	if(cg_verboseParse.integer){CG_Printf("Parse completed succesfully.\n");}
	return qtrue;
}
